...
#define MQTT_TOPIC "image_topic/0610"
...
void on_connect(struct mosquitto *mosq, void *obj, int reason_code)
{
    ...
    mosquitto_subscribe(mosq, NULL, MQTT_TOPIC, 1); // 订阅主题
}
...
int main()
{
    ...
    vfmt.fmt.pix.width = 1920;                // 设置摄像头采集参数，分辨率
    ...
    while (!stop)
    {
        ...
        printf("readbuffer.index=%d,jpeg_size=%lf MB\n", readbuffer.index,(double)jpeg_size/(1024*1024));
        for (int i = 0 ; i < jpeg_size ; i++)
        {
            printf("%d",jpeg_data[i]);
        }
